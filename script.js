...
particlesJS("particles-js", {
    particles: {
        ...
        line_linked: { enable: true, distance: 150, color: "#FFE55C", opacity: 0.4, width: 1 },
...
// Sistema de Som
const sounds = {
    hover: new Howl({ src: ['path/to/hover.mp3'], volume: 0.3 }),
    click: new Howl({ src: ['path/to/click.mp3'], volume: 0.4 }),
    reveal: new Howl({ src: ['path/to/reveal.mp3'], volume: 0.3 })
};
...
function createResponsiveConstellationLines() {
    ...
    container.innerHTML = '';
    const numberOfLines = window.innerWidth <= 768 ? 8 : 15;
    
    for(let i = 0; i < numberOfLines; i++) {
        const line = document.createElement('div');
        line.className = 'constellation-line';
        const width = window.innerWidth <= 768 ? Math.random() * 100 + 30 : Math.random() * 150 + 50;
...
function initRevealAnimations() {
    gsap.utils.toArray('.reveal-section').forEach((section) => {
        ...
        tl.set(section, { opacity: 1, visibility: 'visible' })
          .from(section.children, { y: 50, opacity: 0, duration: 1, stagger: 0.2, ease: 'power3.out' });
    });
}
...
function animateMetric(metric) {
    ...
    confetti({ particleCount: 100, spread: 70, origin: { y: 0.6 }, colors: ['#FFD700', '#FDB931', '#ffffff'] });
...
function createEnergyEffect(element) {
    ...
    gsap.to(energy, { scale: 2, opacity: 0, duration: 1.5, repeat: -1 });
}
...
function initHoverEffects() {
    ...
    document.querySelectorAll('.cta-button').forEach(button => {
        button.addEventListener('mouseenter', () => sounds.hover.play());
        button.addEventListener('click', () => sounds.click.play());
    });
}

function enableFinalCTA() {
    ...
    if (finalCTA && ctaSection) {
        ...
        // Adiciona um evento de clique para parar a propagação
